...
// implements (i.e. appears in the available set), so Backend() never lies about
// running SIMD.
...
func TestSelectBackendInvalidFallsBackToScalar(t *testing.T) {
	for _, force := range []string{"unknown", "AVX2", "neon ", "sha", "0"} {
		got := selectBackend(force)
...
// families) always resolve to scalar rather than a backend that lies about its
// width.
...
func TestSelectBackendForcedScalar(t *testing.T) {
	...
	if got.name != "scalar" || got.lanes != 1 || got.hash32 == nil {
...
// TestForcedBackendHashWorks invokes each available backend's wired hash32
// function pointer directly, hashing exactly one full lane group, to ensure the
// constructor is wired to a kernel that produces correct digests. A full lane
// group is used because the public dispatch only ever calls a backend with a
// multiple of its lane count.
func TestForcedBackendHashWorks(t *testing.T) {
	for _, name := range availableBackendsForTest() {
		withBackend(t, name, func() {
			b := active
			src := randomBytes(t, b.lanes*32)
			dst := make([]byte, b.lanes*Size)
			b.hash32(dst, src, b.lanes)
			for i := 0; i < b.lanes; i++ {
				want := Sum(src[i*32 : (i+1)*32])
				if !bytes.Equal(dst[i*Size:(i+1)*Size], want[:]) {
					t.Fatalf("backend %q hash32 lane %d = %x, want %x", name, i, dst[i*Size:(i+1)*Size], want[:])
				}
			}
		})
	}
